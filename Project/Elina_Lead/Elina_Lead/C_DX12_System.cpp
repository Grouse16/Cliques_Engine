...
bool C_DX12_System::M_DirectX12_Init(void)
{
	...
	{
		// ☆ 変数宣言 ☆ //
		RENDERING::API::DX12::DX12_INSTANCE::C_DX12_Rendering_Screen_System * rendering_screen_system = new RENDERING::API::DX12::DX12_INSTANCE::C_DX12_Rendering_Screen_System();    // 新しいレンダリング先のレンダリング画面システムのアドレス

		RENDERING::API::CREATE::S_Create_Render_Screen_Inform create_screen_inform;    // 画面バッファの生成用情報
		...
		create_screen_inform.m_screen_texture_setting = RENDERING::API::CREATE::E_SCREEN_TEXTURE_SETTING::e_TEXTURE_2D;        // テクスチャの仕様
	...
	// レンダリングAPIは存在しているので終了しているフラグを下げる
...
//☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆//
// 詳細   ： レンダリング画面をクリアする
// 引数   ：S_DX12_Render_Target_View & レンダーターゲットビュー情報の参照
// 戻り値 ：void
//☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆//
...
//☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆//
// 詳細   ：レンダリング画面をセットする
// 引数   ：C_DX12_Rendering_Screen_System * & レンダーターゲットビュー情報の参照
// 戻り値 ：void
//☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆//
void C_DX12_System::M_Set_Render_Target_View(RENDERING::API::DX12::DX12_INSTANCE::C_DX12_Rendering_Screen_System * & in_render_target_view)
{
	...
	// ビューのバインド
	mpr_variable->s_command.list->OMSetRenderTargets(in_render_target_view->m_rendering_screen_sum, &in_render_target_view->m_render_target_view.buffer_list[0].handle, TRUE, &mpr_variable->s_render.now_depth_stencil_address_handle->handle);
	mpr_variable->s_render.now_screen_address_handle = &in_render_target_view->m_render_target_view.buffer_list[0];
...
{
	mpr_variable->s_command.list->OMSetRenderTargets(mpr_variable->s_render.now_rendering_screen_sum, &mpr_variable->s_render.now_screen_address_handle->handle, TRUE, &in_depth_stencil_buffer.handle);
...
{
	// ☆ 変数宣言 ☆ //
	RENDERING::API::DX12::DX12_INSTANCE::C_DX12_Input_Layout_Setting_System input_setting_system;    // 入力レイアウト設定用システム
...
{
	// ☆ 変数宣言 ☆ //
	UINT64 result_fence = mpr_variable->s_command.fence_value[mpr_variable->s_command.num_back_screen]; // 結果のフェンス値を受け取る
	...
	{
		// このFenceにおいて、フェンス値になったらフェンスイベントを実行することを指定
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Rendering_Graphics_Setting_Inform * dx12_pipeline_inform = new DX12_INSTANCE::C_DX12_Rendering_Graphics_Setting_Inform(); // DX12用の描画用情報
...
{
	// ☆ 変数宣言 ☆ //
	const DX12_INSTANCE::C_DX12_Rendering_Graphics_Setting_Inform * dx12_pipeline_inform = reinterpret_cast<const DX12_INSTANCE::C_DX12_Rendering_Graphics_Setting_Inform * > (in_pipeline_inform.get());   // 描画パイプライン用情報をDX12用へキャストした結果のアドレス
...
//☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆//
// 詳細   ：レンダリング画面を生成する
// 引数   ：unique_ptr<C_Rendering_Screen_Data_Base> & 設定先のレンダリング画面の参照, S_Create_Render_Screen_Inform & レンダリング画面を生成するための情報の参照（const）
// 戻り値 ：bool 成功時のみtrue
//☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆//
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Rendering_Screen_System * dx12_rendering_screen = new DX12_INSTANCE::C_DX12_Rendering_Screen_System(); // DX12用のレンダリング画面
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Rendering_Screen_System * dx12_rendering_screen = reinterpret_cast<DX12_INSTANCE::C_DX12_Rendering_Screen_System*>(in_rendering_screen.get());   // レンダリング画面情報をDX12用へキャストした結果のアドレス
...
// 引数   ：int レンダリング画面番号, int 設定先のテクスチャのスロット番号, unique_ptr<C_Rendering_Screen_Data_Base> & レンダリング画面システムの参照
// 戻り値 ：void
//☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆=☆//
...
{
	// ☆ 変数宣言 ☆ //
	const DX12_INSTANCE::C_DX12_Rendering_Screen_System * dx12_rendering_screen = reinterpret_cast<const DX12_INSTANCE::C_DX12_Rendering_Screen_System * >(in_delete_rendering_screen);   // レンダリング画面情報をDX12用へキャストした結果のアドレス
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Depth_Stencil_Buffer_System * dx12_depth_stencil_screen = new DX12_INSTANCE::C_DX12_Depth_Stencil_Buffer_System(); // DX12用の深度ステンシルバッファ
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Depth_Stencil_Buffer_System * dx12_depth_stencil_screen = reinterpret_cast<DX12_INSTANCE::C_DX12_Depth_Stencil_Buffer_System * >(in_clear_depth_stencil_buffer.get()); // DX12用の深度ステンシルバッファ
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Depth_Stencil_Buffer_System* dx12_depth_stencil_screen = reinterpret_cast<DX12_INSTANCE::C_DX12_Depth_Stencil_Buffer_System*>(in_depth_stencil.get()); // DX12用の深度ステンシルバッファ
...
{
	// ☆ 変数宣言 ☆ //
	const DX12_INSTANCE::C_DX12_Depth_Stencil_Buffer_System * dx12_depth_stencil_screen = reinterpret_cast<const DX12_INSTANCE::C_DX12_Depth_Stencil_Buffer_System*>(in_deleted_depth_stencil_buffer); // DX12用の深度ステンシルバッファ
...
{
	mpr_variable->s_command.list->OMSetRenderTargets(mpr_variable->s_render.now_rendering_screen_sum, &mpr_variable->s_render.now_screen_address_handle->handle, TRUE, nullptr);
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Vertex_Setting_Inform * dx12_vertex_inform = new DX12_INSTANCE::C_DX12_Vertex_Setting_Inform();   // DX12用の頂点用情報
...
{
	// ☆ 変数宣言 ☆ //
	const DX12_INSTANCE::C_DX12_Vertex_Setting_Inform * dx12_vertex_inform = reinterpret_cast<const DX12_INSTANCE::C_DX12_Vertex_Setting_Inform*> (in_vertex_inform.get());   // 頂点用情報をDX12用へキャストした結果のアドレス
...
{
	// ☆ 変数宣言 ☆ //
	const DX12_INSTANCE::C_DX12_Vertex_Setting_Inform * dx12_vertex_inform = reinterpret_cast<const DX12_INSTANCE::C_DX12_Vertex_Setting_Inform*> (in_draw_vertex.get());   // 頂点用情報をDX12用へキャストした結果のアドレス
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Index_Setting_Inform * dx12_index_inform = new DX12_INSTANCE::C_DX12_Index_Setting_Inform();   // DX12用のインデックス用情報
...
{
	// ☆ 変数宣言 ☆ //
	const DX12_INSTANCE::C_DX12_Index_Setting_Inform * dx12_index_inform = reinterpret_cast<const DX12_INSTANCE::C_DX12_Index_Setting_Inform * >(in_draw_index.get());   // インデックス用情報をDX12用へキャストした結果のアドレス
...
{
	// ☆ 変数宣言 ☆ //
	const DX12_INSTANCE::C_DX12_Index_Setting_Inform * dx12_index_inform = reinterpret_cast<const DX12_INSTANCE::C_DX12_Index_Setting_Inform * >(in_index_data.get());   // インデックス用情報をDX12用へキャストした結果のアドレス
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Constant_Setting_Inform* dx12_constant_inform = new DX12_INSTANCE::C_DX12_Constant_Setting_Inform();   // DX12用の定数用情報
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Constant_Setting_Inform * dx12_constant_inform = reinterpret_cast<DX12_INSTANCE::C_DX12_Constant_Setting_Inform * >(in_constant_inform.add_constant_buffer);   // 描画用情報をDX12用へキャストした結果のアドレス
...
{
	// ☆ 変数宣言 ☆ //
	DX12::DX12_INSTANCE::C_DX12_Constant_Setting_Inform * dx12_constant_buffer = reinterpret_cast<DX12::DX12_INSTANCE::C_DX12_Constant_Setting_Inform * >(in_constant_buffer.get());    // 定数バッファ情報をDX12用へキャストした結果のアドレス
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Rendering_Texture_Setting_Inform* dx12_texture_inform = new DX12_INSTANCE::C_DX12_Rendering_Texture_Setting_Inform(); // DX12用の描画用情報
...
{
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Rendering_Texture_Setting_Inform * dx12_texture_inform = reinterpret_cast<DX12_INSTANCE::C_DX12_Rendering_Texture_Setting_Inform*>(in_texture_inform.add_texture_data); // 描画用情報をDX12用へキャストした結果のアドレス
...
{
	// ☆ 変数宣言 ☆ //
	DX12::DX12_INSTANCE::C_DX12_Rendering_Texture_Setting_Inform * dx12_texture = reinterpret_cast<DX12::DX12_INSTANCE::C_DX12_Rendering_Texture_Setting_Inform * >(in_texture.get());    // テクスチャ情報をDX12用へキャストした結果のアドレス
...
bool C_DX12_System::M_Create_Font_Data(std::unique_ptr<INSTANCE::C_Rendering_Font_Inform_Base> & in_font_data, const CREATE::S_Create_Font_Data_Inform & in_create_inform) const
{
	...
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Font_Data_Set * new_font_data_handler = new DX12_INSTANCE::C_DX12_Font_Data_Set();   // 新しいフォントデータ用の設定
...
bool C_DX12_System::M_Set_Font_To_Texture_Map(PAKAGE::FONT::S_Make_Font_To_Graph_Package & in_set_inform) const
{
	...
	// ☆ 変数宣言 ☆ //
	DX12_INSTANCE::C_DX12_Font_Data_Set * dx12_font_data = reinterpret_cast<DX12_INSTANCE::C_DX12_Font_Data_Set * >(in_set_inform.attach_font_data->get());   // ＤＸ１２用フォントデータへの変換
