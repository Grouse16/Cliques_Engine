...
// ☆ 多重インクルードガード ☆ //
#ifndef D_INCLUDE_GUARD_C_ENGINE_INPUT_MANAGER_H_FILE
#define D_INCLUDE_GUARD_C_ENGINE_INPUT_MANAGER_H_FILE
...
namespace GAME::INPUT
{
	...
	private:
		...
		struct SPr_Variable
		{
			...
			// マウスの判定用の情報をまとめた構造体
			struct S_Engine_Mouse_Inform
			{
				bool flg_clicking = false;			// クリック中のみたつフラグ
				bool flg_double_clicking = false;	// ダブルクリック中のみたつフラグ
				bool flg_dragging = false;			// ドラッグ操作中のみたつフラグ

				GAME::INPUT::COLLISION::C_Engine_Input_Collision * clicking_collision_add = nullptr;	// クリック中の判定のアドレス
				GAME::INPUT::COLLISION::C_Engine_Input_Collision * drag_overed_add = nullptr;			// ドラッグ中のカーソルの下にある判定のアドレス
		...
		static std::unique_ptr<C_Engine_Input_Manager> m_this;	// インスタンス化するための実体（シングルトン用）
...
#endif // !D_INCLUDE_GUARD_C_ENGINE_INPUT_MANAGER_H_FILE
