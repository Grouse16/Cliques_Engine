...
void C_Game_Time_Manager::M_Create_New_Time_Line(std::string in_set_time_line_name, float in_set_speed)
{
	...
	// ☆ 変数宣言 ☆ //
	int new_time_line_number = m_this->mpr_variable.time_line_list.size();	// 新しいタイムラインの番号
...
{
	// ☆ 変数宣言 ☆ //
	int set_delete_name_slot = m_this->mpr_variable.delete_time_line_list.size();	// 設定先のタイムライン番号
