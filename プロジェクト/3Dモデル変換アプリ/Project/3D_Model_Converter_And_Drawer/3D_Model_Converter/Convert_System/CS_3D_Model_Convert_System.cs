...
namespace _3D_Model_Converter_And_Drawer
{
	...
	internal class CS_3D_Model_Convert_System
	{
		...
		private static void M_Model_Convert(Scene in_scene, ref Form_Announce_Bord in_form_announce_bord)
		{
			...
			// 頂点インデックス情報を書き込む
			M_Write_Vertex_Index_Data(in_scene, ref write_file_data, vertex_to_shrunk_vertex_number_list);
		...
		private static void M_Adjust_Vertex_Weight(ref List<CS_Bone_Weight_Data> in_vertex_weight_list)
		{
			...
			while (weight_sum >= 1.0)
			{
				...
				// ウェイト値の合計を計算
				foreach (CS_Bone_Weight_Data weight_data in in_vertex_weight_list)
				{
		...
		// 頂点インデックス情報を書き込む　引数：モデルのデータ, 書き込むデータの参照, 頂点の変換後の番号リスト
		private static void M_Write_Vertex_Index_Data(Scene in_scene, ref List<string> in_write_file_data, List<int> vertex_to_shrunk_vertex_number_list)
		{
			// ☆ 変数宣言 ☆ //
			int vertex_index_sum = 0;   // 頂点インデックスの合計数
			int now_mesh_vertex_index_slot_offset = 0;	// 現在の頂点インデックスの開始位置


			// 頂点インデックスの合計数を計算
			foreach (Mesh l_now_mesh in in_scene.Meshes)
			{
				// フェース数分繰り返す
				foreach (Face l_now_face in l_now_mesh.Faces)
				{
					vertex_index_sum += l_now_face.Indices.Count;
				}
			}

			// 頂点インデックスの合計数を書き込む
			in_write_file_data.Add("INDEXSUM:" + vertex_index_sum.ToString());
			...
			foreach (Mesh l_now_mesh in in_scene.Meshes)
			{
				...
				{
					// 頂点インデックスデータを書き込む
					foreach (int l_now_vertex_index in l_now_face.Indices)
					{
						in_write_file_data.Add(vertex_to_shrunk_vertex_number_list[now_mesh_vertex_index_slot_offset + l_now_vertex_index].ToString());
					}
				}

				// 経過した面の数分次のメッシュの頂点インデックスデータ開始番号を移動
				now_mesh_vertex_index_slot_offset += l_now_mesh.Vertices.Count;
			}
		...
		private static void M_Write_Mesh_Data(Scene in_scene, ref List<string> in_write_file_data, List<int> vertex_to_shrunk_vertex_number_list)
		{
			...
			{
				// ☆ 変数宣言 ☆ //
				int now_mesh_vertex_index_sum = 0;	// 現在の面の頂点インデックス数
				...
				{
					// 現在のメッシュのインデックス数を加算
					now_mesh_vertex_index_sum += face.Indices.Count;
				}

				// 頂点インデックスデータの開始位置と終了位置を記録
				in_write_file_data.Add(now_mesh_vertex_index_slot_offset.ToString());
				in_write_file_data.Add((now_mesh_vertex_index_slot_offset + now_mesh_vertex_index_sum - 1).ToString());

				// 頂点インデックスデータの開始位置を移動
				now_mesh_vertex_index_slot_offset += now_mesh_vertex_index_sum;
